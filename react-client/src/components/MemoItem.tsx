...
export const MemoItem: React.FC<MemoItemProps> = ({
  ...
  const canEdit = memo.insertUser?.id === storedUser?.id || memo.sharedUsers?.some((u) => u.id === storedUser?.id && u.shareType === 'edit');
        ...
        <div className="relative top-0 right-0 h-full flex flex-col items-center justify-center space-y-4 p-2">
          {isEditing ? (
            ...
            <>
              ...
              {memo.insertUser?.id === storedUser?.id && isAnalyzableFile() && (
                <button
