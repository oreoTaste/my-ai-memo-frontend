...
export const MemoRouter = () => {
  ...
  const [selectedUsers, setSelectedUsers] = useState<MemoUser[]>([]);
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      if (response.data.result) {
        let searchedUser = response.data.users as MemoUser[];
        setSearchUserResults(searchedUser|| []);
      } else {
        setSearchUserResults([]);
      }
    } catch (error) {
  ...
  const updateMemo = async (memoId: number, updatedContent: Partial<Memo>) => {
    ...
    if (
      ...
      JSON.stringify(prevMemo?.sharedUsers?.map((u) => u.id)) ===
        JSON.stringify(updatedContent.sharedUsers?.map((u) => u.id))
    ) {
      cancelMemo(memoId);
      return;
    }
    let updateMemo = {
      ...
      sharedIds: JSON.stringify(updatedContent.sharedUsers?.map((u) => u.id)),
    }; /* 업데이트할 메모 */
    try {
  ...
  const deleteMemo = async (seq: number) => {
    ...
    const isOwnMemo = memoToDelete.insertUser?.id === storedUser?.id /* 현재 사용자 ID */; // 현재 사용자 ID와 비교 로직 필요
          ...
          >
            {searchUserResults.map((user) => (
              <div
                ...
                className={`p-3 flex justify-between items-center cursor-pointer hover:${isDarkMode ? "bg-gray-600" : "bg-gray-100"} ${
        ...
        {selectedUsers.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {selectedUsers.map((user) => (
              <div
                key={user.id}
                className={`inline-flex items-center px-2 py-1 rounded-full text-sm font-medium transition-all duration-200 ${isDarkMode ? "bg-indigo-700 text-white" : "bg-indigo-100 text-indigo-800"}`}
              >
                {user.name} ({user.loginId})
                <button
                  onClick={(e) => {
